import { inject, Injectable } from '@angular/core';
import { ProvidedServicesApiService } from '../provided-services/shared/services/provided-services.api.service';
import { StorageService } from '../../../core/services/storage.service';
import { Router } from '@angular/router';
import { ProvidedServicesComponent } from '../provided-services/provided-services.component';
import { CustomersComponent } from './customers.component';
import { SubscribersApiService } from './shared/services/subscribers.api.service';

@Injectable({
  providedIn: 'root',
})
export class CustomersService {
  private service: SubscribersApiService = inject(SubscribersApiService);
  private storage: StorageService = inject(StorageService);
  private router: Router = inject(Router);
  component: CustomersComponent;
  constructor() {}

  getAll() {
    let st = this.storage.getObject('authResponse');
    this.service.getAllByCompanyId(st.id).then((resp: any) => {
      console.log(resp.data);
      this.component.items = resp.data.map((item: any) => ({
        ...item,
        name:item.firstName + ' ' + item.lastName,
        status: item.active,
        // price: '$' + item.price,
      }));
    });
  }

  setCols() {
    this.component.cols = [
      { field: 'name', header: 'Name' },
      { field: 'phoneNumber', header: 'Phone number' },
      { field: 'status', header: 'Status', width: '200px' },
      { field: 'view', header: 'Actions', width: '200px' },
    ];
  }

  tableActionHandler(e: any) {
    switch (e.type) {
      case 1:
        this.router.navigate(['/main/company/customers/upsert', 'create']);
        break;
      case 4:
        this.router.navigate(['/main/company/customers/details', e.data.id]);
        break;
    }
  }
}
